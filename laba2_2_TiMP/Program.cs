...
namespace laba2_2_TiMP
{
    class Program
    {
        ...
        const string pathToLibCpp = "Resources/MathLibrary.dll";
        const string pathToLibLazarus = "Resources/DllLazarus32.dll";
        const string pathToLibC = "Resources/dllC32.dll";
        #endregion
        ...
        #region DLL Lib_Cpp
        [DllImport(pathToLibCpp, CallingConvention = CallingConvention.Cdecl, EntryPoint = entryPointTheFunc)]
        static public extern double TheFuncCpp(double x);


        [DllImport(pathToLibCpp, CallingConvention = CallingConvention.StdCall, EntryPoint = entryPointFuncName)]
        static public extern IntPtr FuncNameCpp();
        #endregion

        #region DLL Lib_Lazarus
        [DllImport(pathToLibLazarus, CallingConvention = CallingConvention.Cdecl, EntryPoint = entryPointTheFunc)]
        static public extern double TheFuncLazarus(double x);


        [DllImport(pathToLibLazarus, CallingConvention = CallingConvention.StdCall, EntryPoint = entryPointFuncName)]
        static public extern IntPtr FuncNameLazarus();
        #endregion

        #region DLL Lib_C
        [DllImport(pathToLibC, CallingConvention = CallingConvention.Cdecl, EntryPoint = entryPointTheFunc)]
        static public extern double TheFuncC(double x);


        [DllImport(pathToLibC, CallingConvention = CallingConvention.StdCall, EntryPoint = entryPointFuncName)]
        static public extern IntPtr FuncNameC();
        #endregion

        #endregion

        private static Func<double, double>[] arrTheFunc = new Func<double, double>[]
            {TheFunc0, TheFunc1, TheFunc2, TheFunc3, TheFunc3_1, TheFunc3_2, TheFuncCpp, TheFuncLazarus, TheFuncC};

        private static Func<IntPtr>[] arrFuncName = new Func<IntPtr>[]
           {FuncName0, FuncName1, FuncName2, FuncName3, FuncName3_1, FuncName3_2, FuncNameCpp, FuncNameLazarus, FuncNameC};


        static void Main(string[] args)
        {
            Console.WriteLine(
                $"Функции на выбор:{Environment.NewLine}" +
                $"0.TheFunc0{Environment.NewLine}" +
                $"1.TheFunc1{Environment.NewLine}" +
                $"2.TheFunc2{Environment.NewLine}" +
                $"3.TheFunc3{ Environment.NewLine}" +
                $"4.TheFunc3_1{Environment.NewLine}" +
                $"5.TheFunc3_2{Environment.NewLine}" +
                $"6.TheFuncCpp{Environment.NewLine}" +
                $"7.TheFuncLazarus{Environment.NewLine}" +
                $"8.TheFuncC"
                );
